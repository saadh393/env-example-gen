...
program
  ...
  .option('-m, --multi', 'Generate templates for every .env* file in the current directory.')
...
async function handleMultiMode(): Promise<void> {
  ...
  if (envFiles.length === 0) {
    throw new EnvTemplateError('No .env files were found in the current directory.', 'NO_ENV_FILES');
  }
  ...
  if (failureCount > 0) {
    console.error(chalk.red(`✖ Failed to process ${failureCount} file(s).`));
    process.exitCode = 1;
  }
}
...
function logSuccess(result: TemplateGenerationResult): void {
  ...
  console.log(
    chalk.gray(
      `→ wrote ${relativeOutput} (${result.variableCount} variable${result.variableCount === 1 ? '' : 's'})`
    )
  );
}
